...
- 미들웨어는 DB 를 쓰지 않으므로 쿠키의 **존재**만 본다(빠른 1차 차단). 세션의 **유효성**은 페이지·API 에서 검증한다. 그래서 `/login` 에서 "쿠키가 있으니 대시보드로" 라고 되돌리면 안 된다 — 만료·DB 초기화로 무효가 된 쿠키가 남아 있으면 레이아웃이 다시 `/login` 으로 보내 무한 이동이 된다. `/login` 의 되돌리기는 세션을 실제로 확인하는 페이지 컴포넌트가 맡는다.
- 로그인 후 이동 경로 `next` 는 같은 사이트의 경로만 허용한다(`/` 로 시작하고 `//`, `/\` 가 아님). 열린 리다이렉트로 피싱 사이트에 보내는 것을 막는다.
- 테스트: `apps/forms/tests/public-form.test.ts` (헤더/쿠키 검증), `e2e/03-isolation.spec.ts` (악성 템플릿이 실제 브라우저에서 관리자 API/쿠키에 접근 못함, sandbox 속성, CSRF 403), `e2e/01-auth.spec.ts` (무효 쿠키, 외부 `next` 차단).
